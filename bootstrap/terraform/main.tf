...
resource "google_project_service" "this" {
  ...
  depends_on = [google_project_iam_member.group]
}
...
resource "time_sleep" "manager_rights" {
  ...
  triggers = {
    services_md5 = md5(join(",", keys(google_project_iam_member.group)))
  }

  depends_on = [
    google_project_iam_member.group
  ]
}
...
# Grant the group IAM roles for the Terraform IaC
# service account impersonation.
resource "google_service_account_iam_member" "this" {
  for_each = toset(local.group_service_account_iam_roles)
...
resource "time_sleep" "final" {
  ...
  triggers = {
    services_md5 = md5(join(",", concat(local.group_iam_roles, tolist([google_storage_bucket.this.id]))))
  }
