...
function print_usage_and_exit() {
	cat <<-END_OF_DOC
		Bahlsen Project Factory Bootstrapper

		Usage:
		  $0 -p [GCP_PROJECT_ID] -g [GROUP] [ADDITIONAL OPTIONS]

		Options:
		  -p <string> (required) GCP Project ID
		  -g <string> (required) Administrative group used to grant further permissions to.
		  ...
		  -r <string> (optional) GitHub repository in the format '<owner/org>/<reponame>'. If
...
# Parameter parsing
while getopts ":p:s:b:o:g:r:t:hi" OPT; do
	case $OPT in
	p)
		GCP_PROJECT_ID="${OPTARG}"
		;;
	g)
		GCP_IAM_GROUP="${OPTARG}"
		;;
	s)
...
SA_NAME="${SA_NAME_PARAM:-terraform}"
...
ACTIVE_GCLOUD_ACCOUNT="$(gcloud --quiet auth list --format='value(account)' --filter='status:ACTIVE')"
...
if [ "${GCP_PROJECT_ID:-notset}" = "notset" ]; then
	...
	VARIABLES_EXTRACTED=$(echo "${PROJECT_DATA}" | jq -r '@sh "GCP_PROJECT_NAME=\(.name) GCP_PROJECT_NUMBER=\(.projectNumber) GCP_PROJECT_LIFECYCLE=\(.lifecycleState)"')
...
# TODO: More validation, for example try to add group project
if [ "${GCP_IAM_GROUP:-notset}" = "notset" ]; then
	log_error "Missing GROUP parameter! Make sure the '-g' parameter is correctly set." 1>&2
	exit 1
fi
...
cat <<-EOF
	...
	The active account needs the 'Project IAM Admin' role inside the project.
	We assume that the permissions are granted to your active account, most
	likely by folder level permissions.
	${TEXT_ALL_OFF}
EOF
