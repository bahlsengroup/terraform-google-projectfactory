...
locals {
  ...
  project_iam_google_default_bindings = local.compute_enabled ? {
    "roles/editor" = {
      role      = "roles/editor"
      condition = null
  ...
  project_iam_roles_combined = toset(compact(concat(
    ...
    keys(local.project_iam_google_default_bindings),
  ...
  project_iam_bindings_combined = { for role in local.project_iam_roles_combined :
    role => {
      role = distinct(compact(concat(
        [try(local.project_iam_policy_bindings[role].role, "")],
        [try(local.project_iam_custom_role_bindings[role].role, "")],
        [try(local.project_iam_service_account_bindings[role].role, "")],
        [try(local.project_iam_google_default_bindings[role].role, "")],
        [try(local.project_iam_non_authoritative_role_bindings[role].role, "")],
        [try(local.project_iam_pam_bindings[role].role, "")],
      )))[0]
      # filter out deleted principals as they cant be used in IAM policies
      members = [for member in distinct(compact(concat(
        try(local.project_iam_policy_bindings[role].members, []),
        try(local.project_iam_custom_role_bindings[role].members, []),
        try(local.project_iam_service_account_bindings[role].members, []),
        try(local.project_iam_google_default_bindings[role].members, []),
        try(local.project_iam_non_authoritative_role_bindings[role].members, []),
        try(local.project_iam_pam_bindings[role].members, []),
      ))) : member if !startswith(member, "deleted:")]
      condition = one([for condition in flatten([
        [try(local.project_iam_policy_bindings[role].condition, null)],
        [try(local.project_iam_custom_role_bindings[role].condition, null)],
        [try(local.project_iam_service_account_bindings[role].condition, null)],
        [try(local.project_iam_google_default_bindings[role].condition, null)],
        [try(local.project_iam_non_authoritative_role_bindings[role].condition, null)],
        [try(local.project_iam_pam_bindings[role].condition, null)],
